...
namespace Esper.FeelSpeak
{
    ...
    public static class FeelSpeak
    {
        ...
        /// <summary>
        /// Gets all dialogue graphs that are a part of a specific dialogue group.
        /// </summary>
        /// <param name="groupId">The dialogue group ID.</param>
        /// <returns>All dialogue graphs with the group ID.</returns>
        public static List<DialogueGraph> GetDialogueGraphs(int groupId)
        {
            List<DialogueGraph> dialogues = new();

            var records = FeelSpeakDatabase.GetDialogueRecords(groupId);

            foreach (var record in records)
            {
                dialogues.Add(GetDialogueGraph(record));
            }
